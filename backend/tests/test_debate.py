...
async def collect(query, participants, keys):
    """Run a debate to completion and return the parsed event dicts."""
    events = []
    async for frame in debate.run_debate(query, participants, keys):
...
class DebateEngineTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_fast_path_consensus_needs_no_judge(self):
        ...
        self.assertNotIn(prompts.JUDGE_SYSTEM, [s for _, _, s in calls])
    ...
    async def test_same_provider_two_models_both_debate(self):
        ...
        round_calls = [(p, m) for p, m, s in calls if s == prompts.INITIAL_SYSTEM]
    ...
    async def test_judge_rotates_each_round(self):
        ...
        judge_calls = [pid_of(p, m) for p, m, s in calls if s == prompts.JUDGE_SYSTEM]
